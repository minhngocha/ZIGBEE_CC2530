...
void user_debug_loopback_data(uint8 port, uint8 event);
void user_debug_handkey_uart(uint8 port, uint8 event);
...
void GenericApp_SendTheMessage( void )
{
 // char theMessageData[] = "Hello World     \r\n";
  ...
  count_transmit++;
   ...
   sprintf(TX_transmit,"MINHNEO ED %d \r\n",count_transmit);
#endif
#if ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ROUTER  
#if ROUTER1
   sprintf(TX_transmit,"MINHNEO RT 111 %d \r\n",count_transmit);
#else
   sprintf(TX_transmit,"MINHNEO RT 222 %d \r\n",count_transmit);
#endif
#endif
  user_debug(TX_transmit);

  /*
  if ( AF_DataRequest( &SerialApp_TxAddr, (endPointDesc_t *)&SerialApp_epDesc,
                       SERIALAPP_CLUSTERID1,
                       (byte)osal_strlen( TX_transmit ) + 1,
                       (byte *)&TX_transmit,
                       &SerialApp_MsgID,
                       AF_DISCV_ROUTE, AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )*/
      SerialApp_user.addrMode = (afAddrMode_t) Addr16Bit;
      SerialApp_user.endPoint = SERIALAPP_ENDPOINT;
      SerialApp_user.addr.shortAddr = 0xFFFC;  
...
void user_debug_loopback_data(uint8 port, uint8 event)
{
  ...
  while(Hal_UART_RxBufLen(port))
  {
    ...
    HalUARTWrite( SERIAL_APP_PORT, (uint8 *) &icho, 1 ) ;
  }
}
...
void user_debug_handkey_uart(uint8 port, uint8 event)
{
...
#if ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ENDDEVICE || ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ROUTER
    if( osal_start_timerEx(SerialApp_TaskID, PERIODIC_TIME_EVT ,3000) == SUCCESS)
    {
  ...
  if(icho == 'b')
  {
    SerialApp_HandleKeys( 0, HAL_KEY_SW_4 );
  }
  
  if(icho == 'x')
  {
  char TX_transmit[25];
  sprintf(TX_transmit,"SEND TO COORDINATOR \r\n");
  user_debug(TX_transmit);
...
void SerialApp_Init( uint8 task_id )
{
  ...
  uartConfig.callBackFunc         = user_debug_handkey_uart; 
...
#if ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ENDDEVICE  || ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ROUTER
  osal_start_timerEx(SerialApp_TaskID, START_END_DEVICE,5000);
#endif

#if ZSTACK_DEVICE_BUILD == DEVICE_BUILD_COORDINATOR 
  osal_start_timerEx(SerialApp_TaskID, START_COORDINATOR ,4500);
#endif
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
  ...
  if ( events & PERIODIC_TIME_EVT )
  {
    ...
    osal_start_timerEx(SerialApp_TaskID, PERIODIC_TIME_EVT ,1000);
...
#if ZSTACK_DEVICE_BUILD == DEVICE_BUILD_COORDINATOR
  if ( events & START_COORDINATOR )
  {
    if(count_delay ==1)
    {
    user_debug("Start coordinator\r\n");
    SerialApp_HandleKeys( 0, HAL_KEY_SW_2 );
    osal_stop_timerEx(SerialApp_TaskID, START_COORDINATOR);
    }
    ...
    return ( events ^ START_COORDINATOR );
  }
#endif
...
//    {
      ...
      if ( HalUARTWrite( SERIAL_APP_PORT, pkt->cmd.Data, (pkt->cmd.DataLength-1) ) )  //+1
      {
#if ZSTACK_DEVICE_BUILD == DEVICE_BUILD_COORDINATOR
        char tx_id_RTED[100];
        sprintf(tx_id_RTED,"SerialApp_RxAddr.addr %d \r\nSerialApp_RxAddr.addrMode %d \r\nSerialApp_RxAddr.panId %d \r\n  ",SerialApp_RxAddr.addr,SerialApp_RxAddr.addrMode,SerialApp_RxAddr.panId);
        user_debug(tx_id_RTED);
#endif
